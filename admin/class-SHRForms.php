...
if ( ! class_exists( 'SHRFormsAdmin' ) ) {
	class SHRFormsAdmin {
		  public function __construct() {
            add_action( 'init', array( $this, 'create_post_type' ) );
            add_filter( 'manage_shr_contact_form_posts_columns', array( $this, 'columns_head' ) );
            add_action( 'manage_shr_contact_form_posts_custom_column', array( $this, 'columns_content' ), 10, 2 );
        }

		 public function create_post_type() {
            register_post_type( 'shr_contact_form',
                array(
                    'labels' => array(
                        'name' => __( 'Feedback' ),
	  ...
	  public function columns_head( $defaults ) {
            unset( $defaults['date'] );
            $defaults['submission_email'] = __( 'Email', 'shr-forms' );
            $defaults['submission_website'] = __( 'Website', 'shr-forms' );
            $defaults['submission_excerpt'] = __( 'Excerpt', 'shr-forms' );
            $defaults['date'] = 'Date';
...
$shrformms_admin = new SHRFormsAdmin;